			...
			<div v-if="meetings.length">
				...
				>
					...
					<div class="text-gray-700 mt-2 text-sm">
						<div class="mt-2">
							<b>Schedule/s:</b><br/>
							<div v-for="schedule in meetings[0].schedules">
								{{ formatDateTime(schedule.date + ' ' + schedule.from, 'EEEE, MMMM d, yyyy h:mma') }} - {{ formatDateTime(schedule.date + ' ' + schedule.to, 'h:mma') }}
							</div>
						</div>
						<div class="mt-2">
							<b>Timezone:</b> {{ meetings[0].timezone }}
						</div>
						<div class="mt-2">
							<b>Duration:</b> {{ meetings[0].duration }} minutes<br/>
						</div>
					</div>
					...
					<div v-else class="mt-4 text-center text-black p-1 bg-yellow-300 rounded-md">
						<div>Waiting for Client Response</div>
					</div>

					<div class="mt-4">
						<h3 class="text-lg font-bold mb-2">Client:</h3>
						<p class="text-sm">{{ meetings[0].client.name }} ({{ meetings[0].client.email }})</p>
					</div>
				</div>
			</div>
